...
This repo accompanies my application to EleutherAI SOAR 2026, project I-9
"Tracing Subliminal Learning to Pretraining". I made it to show real interest in
the project, and because I got curious about the question itself. I will keep
adding new steps here until the application decisions come out (July 5, 2026).
...
| Fine-tuning        | Runs | Teacher delta | Contrast (owl minus control)        | Transfer |
|--------------------|------|---------------|-------------------------------------|----------|
| LoRA (160M, 410M)  | 3    | +8 to +10.5   | -0.34 to -0.51                      | no       |
| full FT (160M)     | 4    | +10 to +13    | +0.65 to +1.18, all CIs above zero  | yes      |

The two archived runs (JSONs in `results/`, their exact training data in
`results/gen_cache/`):

- seed 0: contrast **+0.65**, 95% CI [+0.18, +1.16], teacher delta +12.2;
- seed 1: contrast **+1.14**, 95% CI [+0.47, +1.83], teacher delta +12.9.

10k filtered sequences per student, zero owl tokens in the data (checked by
substring search in every run). The LoRA runs and two earlier full-FT runs are in
`LOG.md` with exact numbers; their result files are not committed because they
predate a fix to result naming and labeling (details in the log).
...
Things to keep in mind: one trait (owl) and one model family so far; the teacher
was a bit stronger in the full-FT runs than in the LoRA runs, so method and
strength are not fully separated yet; and full-FT training is not
bit-deterministic on GPU, so the contrast size moves between repeats (+1.18 vs
+0.65 at the same config) while the sign held in all four runs.
...
2. The teacher continues prompts like "The sequence starts with: 182, 818, 725.
   ...
   word appears nowhere in the kept data.
...
PYTHONPATH=src python experiments/strength_probe.py \
  --method full --model EleutherAI/pythia-160m \
  --ind-examples 256 --ind-epochs 3 --ind-lr 5e-5 \
  --student-epochs 3 --student-lr 2e-5 \
  --seed 0 --target owl     # or --seed 1
```

The exact number data of the archived runs is committed (four JSONL files in
`results/gen_cache/`), and the script picks it up and skips generation. So a run
is: teacher induction, two student trainings, eval. About 20-30 minutes on a
...
full config and package versions. Expect a positive contrast with a CI above
zero, not the exact archived digits (see the determinism note above).

A fast pipeline check: `PYTHONPATH=src python experiments/exp0_gate.py --smoke`
...
- `experiments/` - `strength_probe.py` is the main one; the rest are calibration
  and diagnostics that the log mentions.
- `results/` - one JSON per run (metrics plus the full config inside) and the
  data cache of the archived runs.
